...
title CoolHeaded - lock the browser

net session >nul 2>&1
if %errorLevel% neq 0 (
  echo.
  echo   This needs to run as Administrator.
  echo   Right-click install-coolheaded.bat and choose "Run as administrator".
  echo.
  pause
  exit /b 1
)
...
:: Lock CoolHeaded only. Every other extension is untouched.
...
echo     chrome://extensions   CoolHeaded's Remove should be greyed out,
echo                           other extensions unaffected
echo     Right-click the CoolHeaded icon - Remove should be unavailable
echo.
