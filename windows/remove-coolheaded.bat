...
title CoolHeaded - remove protection

net session >nul 2>&1
if %errorLevel% neq 0 (
  echo.
  echo   This needs to run as Administrator.
  echo   Right-click remove-coolheaded.bat and choose "Run as administrator".
  echo.
  pause
  exit /b 1
)
...
echo.
echo   COOLHEADED - REMOVING PROTECTION
echo   ==============================
echo.
echo   To remove CoolHeaded you need the removal code.
echo.
echo   Get it by opening CoolHeaded's settings in your browser, clicking
...
for /f "delims=" %%A in ('powershell -NoProfile -Command "$now=(Get-Date).ToUniversalTime(); $ok=@(); foreach($h in 0,1){ $s=$now.AddHours(-$h).ToString('yyyyMMddHH'); $b=[Text.Encoding]::UTF8.GetBytes('coolheaded-removal-'+$s); $x=[Security.Cryptography.SHA256]::Create().ComputeHash($b); $ok += (($x[0..3] | ForEach-Object { $_.ToString('x2') }) -join '').ToUpper() }; if($ok -contains '%CODE%'.Trim().ToUpper()){'YES'}else{'NO'}"') do set VALID=%%A
...
echo.
echo   The extensions page will work again, and you can remove the CoolHeaded
echo   extension from there in the normal way.
echo.
echo   If you want it back later, run install-coolheaded.bat as Administrator.
echo.
pause
