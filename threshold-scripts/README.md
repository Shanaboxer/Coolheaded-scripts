...
The scripts already know Threshold's extension ID
(`geciepejjdhbcafgbfkfnofjlcaholok`) — there's nothing for you to look up or
type in. You can check it matches the ID on the store listing if you'd like to
verify that the scripts are protecting the extension you actually installed.
...
gets a different extension ID, so the policy won't apply to it.
